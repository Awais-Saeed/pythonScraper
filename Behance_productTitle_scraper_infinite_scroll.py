...
# Wait for page to be loaded properly
time.sleep(5)
...
while itemsVisited < n:
    ...
    newCount = len(driver.find_elements(By.XPATH, "//a[contains(@class,'title')]"))
    ...
    for x in range(oldCount, newCount):
        elem = driver.find_element(By.XPATH, "(//a[contains(@class,'title')])["+str(x+1)+"]").get_attribute("textContent")
        print(elem)
